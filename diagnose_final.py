"""
Final diagnostic with working URL to inspect job card structure.
"""
...
async def final_diagnostic():
    """Final diagnostic with working URL."""
    adapter = NaukriAdapter(browser_type="chrome")

    print("="*80)
    print("FINAL NAUKRI DIAGNOSTIC")
    print("="*80)
    print()
    ...
    try:
        ...
        if job_cards:
            ...
            print(f"First 1500 characters of card HTML:")
            ...
            # Look for alternative selectors by examining all links
            print("Looking for all links in first card...")
            ...
            for i, link in enumerate(all_links):
                ...
                print(f"  Link {i+1}: class='{class_attr}' text='{text[:40]}...' href='{href[:60] if href else 'no href'}...'")
            print()
            ...
            for i, elem in enumerate(title_candidates):
                ...
                print(f"    {i+1}. class='{class_attr}' text='{text[:40]}...'")
            print()
            ...
            for i, elem in enumerate(company_candidates):
                class_attr = await elem.get_attribute('class')
                text = await elem.inner_text()
                print(f"    {i+1}. class='{class_attr}' text='{text[:40]}...'")
            print()
            ...
            for i, elem in enumerate(location_candidates):
                class_attr = await elem.get_attribute('class')
                text = await elem.inner_text()
                print(f"    {i+1}. class='{class_attr}' text='{text[:40]}...'")
            print()

            print("Looking for experience patterns...")
            exp_candidates = await first_card.query_selector_all('[class*="exp"]')
            print(f"  Elements with 'exp' in class: {len(exp_candidates)}")
            for i, elem in enumerate(exp_candidates):
                class_attr = await elem.get_attribute('class')
                text = await elem.inner_text()
                print(f"    {i+1}. class='{class_attr}' text='{text[:40]}...'")
            print()

            print("Looking for salary patterns...")
            sal_candidates = await first_card.query_selector_all('[class*="sal"]')
            print(f"  Elements with 'sal' in class: {len(sal_candidates)}")
            for i, elem in enumerate(sal_candidates):
                class_attr = await elem.get_attribute('class')
                text = await elem.inner_text()
                print(f"    {i+1}. class='{class_attr}' text='{text[:40]}...'")
            print()
